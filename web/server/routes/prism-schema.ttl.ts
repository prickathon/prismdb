...
export default defineEventHandler(async () => {
	...
	for (const binding of json.results.bindings) {
		...
		const o = binding.o;
		if (!s || !p || !o) continue;
		const subject = N3.DataFactory.namedNode(s.value);
		const predicate = N3.DataFactory.namedNode(p.value);
		const object =
			o.type === "uri"
				? N3.DataFactory.namedNode(o.value)
				: N3.DataFactory.literal(
						o.value,
						"datatype" in o ? N3.DataFactory.namedNode(o.datatype) : undefined,
					);
